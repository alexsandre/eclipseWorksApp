...
RUN --mount=type=cache,id=nuget,target=/root/.nuget/packages \
    dotnet publish ./EclipseWorksApp.API/EclipseWorksApp.API.csproj -a ${TARGETARCH/amd64/x64} --use-current-runtime --self-contained false -o /app
...
ENTRYPOINT ["dotnet", "EclipseWorksApp.API.dll"]
